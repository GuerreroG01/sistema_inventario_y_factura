...
import { EndEmploymentModal} from "./EndEmploymentModal";
...
type CurrentEmploymentProps = {
    ...
    endEmployment: (endDate: string) => Promise<EmployeeEmployment>;
};

export function CurrentEmployment({ employment,  onOpenCreateEmployment, endEmployment }: CurrentEmploymentProps) {
    const [ isEndEmploymentModalOpen, setIsEndEmploymentModalOpen ] = useState(false);
    ...
    return (
        <>
                ...
                <div className="flex flex-col gap-4 border-b border-slate-100 p-6 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row">
                        ...
                        {isActive && (
                            <button
                                type="button"
                                onClick={() => setIsEndEmploymentModalOpen(true)}
                                className="inline-flex items-center justify-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-600 transition hover:border-rose-300 hover:bg-rose-100 hover:text-rose-700"
                            >
            ...
            <EndEmploymentModal
                open={isEndEmploymentModalOpen}
                employment={employment}
                onClose={() =>
                    setIsEndEmploymentModalOpen(false)
                }
                onConfirm={endEmployment}
            />
        </>
    );
}