...
import { useInventoryAlertsProducts } from "../../hooks/useInventoryAlertsProducts";
...
export default function InventoryAlertsMetrics() {
    ...
    const handleOpenModal = async (
        ...
        if (
            products.critical.length === 0 &&
            products.exhausted.length === 0
        ) {
            await loadProducts();
        }
...
function InventoryMetricBlock({
    ...
    const styles = {
        critical: {
            ...
            icon: <WarningIcon className="w-5 h-5 text-orange-600" />,
            bg: "bg-orange-50 border-orange-100",
            value: "text-orange-700"
        },
        empty: {
            border: "hover:border-red-200 hover:bg-red-50/20",
            icon: <BoxIcon className="w-5 h-5 text-red-600" />,
